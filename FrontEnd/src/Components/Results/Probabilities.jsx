...
export default function Probabilities({ Probabilities1, Probabilities2 }) {
                        ...
                        {tableData.map((row, i) => (
                            <tr key={i}>
                                ...
                                {row.map((cell, j) => (
                                    <td
                                        key={j}
                                        className={`payoffmatrix-cell ${
                                            cell > 0 ? 'positive' : cell < 0 ? 'negative' : ''
                                        }`}
                                    >
    ...
    return (
        <div className="probabilities-container">
            {showProbabilities1 &&!showProbabilities2 && renderProbabilityTable(Probabilities1, "Computer Probabilities")}
            {showProbabilities1 && showProbabilities2 && renderProbabilityTable(Probabilities1, "Hider Probabilities")}
            {showProbabilities2 && renderProbabilityTable(Probabilities2, "Seeker Probabilities")}
        </div>
    );
}